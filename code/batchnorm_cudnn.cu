...
void BatchNorm::map(void)
{
  ...
  size_t outputSize = sizeof(DATATYPE) * BATCH_SIZE * inputC * inputH * inputW;
...
void BatchNorm::forward(void)
{
  ...
  if (model->isTraining) {
    ...
    checkCUDNN(cudnnBatchNormalizationForwardTraining(
      model->dnn, mode, &alpha, &beta, inputTensor, inputs[0].ptr,
      outputTensor, outputs[0].ptr, biasTensor, scalePtr, biasPtr,
