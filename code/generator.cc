...
public:
  ...
  : OpTemp(1, 1, OpBase::OP_ENLARGECONV), kernelH(_kernelH), kernelW(_kernelW)
  {}
  bool compute(int n, TensorTemp* inputs, int opIdx)
  {
    assert(false);
...
public:
  ...
  : OpTemp(2, 1, OpBase::OP_CONV2D), kernelH(_kernelH), kernelW(_kernelW),
  ...
  bool compute(const TensorTemp& input, const TensorTemp& weight, int opIdx)
  {
    ...
    //if (input.dim[0] != BATCH_SIZE && input.dim[0] != 2 * BATCH_SIZE) return false;
...
int main(int argc, char **argv)
{
  ...
  inputs.push_back(w3);
  // Create 4D tensors
  TensorTemp i1, i2, i3, w4, w5, w6, w7, w8, w9;
  ...
  inputs.push_back(w6);
  init_tensor_temp(w7, "w7", -13, 0, 4, 4, 1, 1);
  inputs.push_back(w7);
  init_tensor_temp(w8, "w8", -13, 0, 4, 4, 1, 1);
  inputs.push_back(w8);
  init_tensor_temp(w9, "w9", -13, 0, 4, 4, 1, 1);
  inputs.push_back(w9);
  // Create 0D scalar tensors
  TensorTemp s0;
  init_tensor_temp(s0, "s0", -16, 0);
  inputs.push_back(s0);
  ...
  // test
  OpTemp* enlarge = new EnlargeConvTemp(3, 3);
  OpTemp* conv3x3 = new Conv2DTemp(3, 3, 1, 1, true, true);
  OpTemp* conv1x1 = new Conv2DTemp(1, 1, 1, 1, true, true);
  assert(enlarge->compute(w7, 0));
  assert(conv3x3->compute(i1, enlarge->outputs[0], 0));
  assert(conv1x1->compute(i1, w7, 0));
  // test
  dfs(0, graph, inputs, ops, hashmap, transfers);
  printf("===================== Generated Transfers =====================\n");
  for (int i = 0; i < transfers.size(); i++)
    if (!(transfers[i].isDuplicate)) {
