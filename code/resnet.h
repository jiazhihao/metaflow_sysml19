...
Tensor BasicBlock(Graph* graph, Tensor input, int outChannels, int stride)
{
...
Graph* ResNet34(Model* model)
{
  ...
  for (int i = 0; i < 3; i++)
    t = BasicBlock(graph, t, 64, 1);
  for (int i = 0; i < 4; i++) {
    int stride = (i==0) ? 2 : 1;
    t = BasicBlock(graph, t, 128, stride);
  }
  for (int i = 0; i < 6; i++) {
    int stride = (i==0) ? 2 : 1;
    t = BasicBlock(graph, t, 256, stride);
  }
  for (int i = 0; i < 3; i++) {
    int stride = (i==0) ? 2 : 1;
    t = BasicBlock(graph, t, 512, stride);
  }
